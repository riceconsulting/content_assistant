...
const Dropdown: React.FC<DropdownProps> = ({ label, value, onChange, options, icon }) => {
  ...
  return (
    <div>
      <label htmlFor={selectId} className="flex items-center text-sm font-medium text-slate-300 mb-2">
        {icon}
      ...
      <div className="relative">
        <select
          ...
          value={value}
          onChange={onChange}
          className="w-full appearance-none bg-slate-700 border border-slate-600 rounded-md py-2 pl-3 pr-10 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm transition"
        >
          {options.map((option) => (
